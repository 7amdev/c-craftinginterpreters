...
{
    if (compiler_current->type == TYPE_SCRIPT)
    {
    ...
    {
        if (compiler_current->type == TYPE_INITIALIZER)
        {
...
static void forStatement()
{
    ...
    int condition_clause_check_point = currentChunk()->count;
    int exitJump = -1;
    if (!match(TOKEN_SEMICOLON))
    {
        expression();
    ...
    int increment_clause_checkpoint;
    if (!match(TOKEN_RIGHT_PAREN))
    {
        int body_statement_checkpoint = emitJump(OP_JUMP);
        increment_clause_checkpoint = currentChunk()->count;
        expression();
        emitByte(OP_POP);
        consume(TOKEN_RIGHT_PAREN, "Expect ')' after for clauses.");

        emitLoop(condition_clause_check_point); // Jump to Conditional check point.
        // loopStart = incrementStart;
        patchJump(body_statement_checkpoint);
    }

    // Compiles the body clause.
    statement();
    emitLoop(increment_clause_checkpoint);
...
static void endScope()
{
    ...
    while (compiler_current->localCount > 0 &&
           compiler_current->locals[compiler_current->localCount - 1].depth > compiler_current->scopeDepth)
    {
        if (compiler_current->locals[compiler_current->localCount - 1].isCaptured)
        {
...
static void namedVariable(Token name, bool canAssign)
{
    ...
    else if ((arg = resolveUpvalue(compiler_current, &name)) != -1)
    {
